...
from src.plugins.service_registry_plugin import ServiceRegistryPlugin
from src.redis_worker.task_types import get_task_definition, validate_task_data, get_plugin_for_task
...
class PluginLoader:
    ...
    async def _load_service_registry(self):
        ...
        try:
            module = importlib.import_module("src.plugins.service_registry_plugin")
    ...
    async def _load_plugin(self, plugin_name: str) -> Optional[BasePlugin]:
        ...
        try:
            ...
            module_map = {
                "ServiceRegistryPlugin": "src.plugins.service_registry_plugin",
                "CacheManagerPlugin": "src.plugins.cache_manager_plugin",
                "MetricsPlugin": "src.plugins.metrics_plugin"
            }
    ...
    async def route_task_to_plugin(self, task_type: str, task_data: Dict[str, Any]) -> PluginExecutionResult:
        ...
        is_valid, error_message = validate_task_data(task_type, task_data.get("data", {}))
        if not is_valid:
            return PluginExecutionResult(
                success=False,
