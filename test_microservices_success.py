...
async def test_service_capabilities():
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        assert "mock_gensim" in available_providers
        assert "mock_spacy" in available_providers
        logger.info("✅ NLP Providers: {}".format(available_providers))
        
        # Test LLM provider info
        response = await client.get("http://localhost:8002/provider")
        assert response.status_code == 200
        provider_info = response.json()
        assert provider_info["type"] == "llm"
        assert len(provider_info["models"]) > 0
        logger.info("✅ LLM Provider: {} with models {}".format(provider_info["name"], provider_info["models"]))
...
async def test_concept_expansion():
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        response = await client.post(
            "http://localhost:8001/providers/mock_gensim/expand",
            json={
                "concept": "action",
                "media_context": "movie",
                "max_concepts": 3,
        ...
        assert "action_related" in str(result["result"]["expanded_concepts"])
        logger.info("✅ NLP Expansion: {} → {}".format("action", result["result"]["expanded_concepts"]))
        ...
        result = response.json()
        assert result["success"] is True
        assert "llm_expanded_thriller" in str(result["result"]["expanded_concepts"])
        logger.info("✅ LLM Expansion: {} → {}".format("thriller", result["result"]["expanded_concepts"]))
...
async def test_service_communication():
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        response = await client.post("http://localhost:8003/services/health-check")
        assert response.status_code == 200
