...
from src.plugins.field_enrichment.conceptnet_keyword_plugin import ConceptNetKeywordPlugin
from src.plugins.field_enrichment.gensim_similarity_plugin import GensimSimilarityPlugin  
from src.plugins.field_enrichment.llm_keyword_plugin import LLMKeywordPlugin
from src.plugins.field_enrichment.spacy_temporal_plugin import SpacyTemporalPlugin
from src.plugins.field_enrichment.heideltime_temporal_plugin import HeidelTimeTemporalPlugin
from src.plugins.field_enrichment.sutime_temporal_plugin import SUTimeTemporalPlugin
from src.plugins.field_enrichment.llm_question_answer_plugin import LLMQuestionAnswerPlugin
from src.plugins.field_enrichment.llm_temporal_intelligence_plugin import LLMTemporalIntelligencePlugin
from src.plugins.field_enrichment.merge_keywords_plugin import MergeKeywordsPlugin
...
class RealWorldTester:
    ...
    async def test_single_plugin(self, plugin_class, plugin_name: str, test_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            result["input_data"] = str(field_value)[:100] + ("..." if len(str(field_value)) > 100 else "")
            ...
            if isinstance(output, dict):
                ...
                for key, value in output.items():
                    if isinstance(value, list):
                        print(f"       {key}: [{len(value)} items] {str(value[:2])[:100]}{'...' if len(value) > 2 else ''}")
